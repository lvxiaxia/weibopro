 ...
 * 存储用户和微博数据
 */
public class FirstInfoExtraction {
...
//			System.out.println(sql);
	...
	public static void readUserMicroblogs() throws Exception{
		//0 1 2 
		File file = new File("data/source/first/microblogs/microblogs0.txt");   
		BufferedInputStream fis = new BufferedInputStream(new FileInputStream(file));    
		BufferedReader reader = new BufferedReader(new InputStreamReader(fis,"utf-8"),5*1024*1024);// 用5M的缓冲读取文本文件  
...
//			readUserIntro();//插入introduction
//			readUserTags();//插入标签
//			readUserProfession();//插入职业信息
//			readUserNetwork();//插入用户关注列表
			readUserMicroblogs();//插入用户微博
